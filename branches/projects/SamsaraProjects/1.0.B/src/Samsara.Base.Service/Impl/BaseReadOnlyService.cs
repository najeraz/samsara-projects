...
namespace Samsara.Base.Service.Impl
{
    public class BaseReadOnlyService<T, TId, TDao, Tpmt> : IBaseReadOnlyService<T, TId, Tpmt> where TDao : IBaseReadOnlyDao<T, TId, Tpmt>
    {
        ...
        public DataTable SearchByParameters(string queryName, Tpmt parameters, bool absoluteColumnNames)
        {
            return this.Dao.SearchByParameters(queryName, parameters, absoluteColumnNames);
        }

        public DataTable SearchByParameters(string queryName, Tpmt parameters)
        {
